...
const forwardFinalTranscript = useCallback(
  async (
    item: TranscriptHelperItem<
      Partial<UserTranscription | AgentTranscription>
    >,
  ) => {
    if (item.status !== TurnStatus.END) {
      return;
    }
    ...
    try {
      const response = await fetch(
        '/api/transcript',
        {
          ...
          body: JSON.stringify({
            incidentId: agoraData.incidentId,
            sessionId,
            speakerId,
            speakerName,
            speakerRole,
      ...
      if (!response.ok) {
        ...
        console.error(
          '[Sentinel] Failed to forward transcript:',
          response.status,
          errorText,
        );
      ...
      console.log(
        '[Sentinel] Transcript forwarded:',
        text,
      );
    } catch (error) {
      console.error(
        '[Sentinel] Transcript forwarding error:',
        error,
      );
    }
  },
  [
    agoraData.incidentId,
    agentUID,
    client.uid,
    sessionId,
  ],
  ...
  (t) => {
    ...
    const latestItem = t[t.length - 1];

    if (latestItem) {
      void forwardFinalTranscript(
        latestItem,
      );
    }
  },
